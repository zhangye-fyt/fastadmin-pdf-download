...
(
	function( $ ) {
		...
		redux.field_objects.repeater.sort_repeaters = function( selector ) {
			...
			selector.find( '.redux-repeater-accordion-repeater' ).each( function( idx ) {
		...
		redux.field_objects.repeater.init = function( selector ) {
			...
			$( selector ).each( function( idx ) {
				...
				var accordian = el.find( ".redux-repeater-accordion" ).accordion( {
					...
					collapsible: true, //active: active,
				...
				el.find( '.redux-repeater-accordion-repeater .bind_title' ).on( 'change keyup', function( event ) {
				...
				el.on( 'click', '.redux-repeaters-remove', function() {
					redux_change( $( this ) );
					var parent                = $( this ).parents( '.redux-container-repeater:first' );
					var gid                   = parent.attr( 'data-id' );
					redux.repeater[gid].blank =
						$( this ).parents( '.redux-repeater-accordion-repeater:first' ).clone( true, true );
					$( this ).parents( '.redux-repeater-accordion-repeater:first' ).slideUp( 'medium', function() {
						$( this ).remove();
				...
				el.on( 'click', '.redux-repeaters-add', function( evt ) {
					evt.preventDefault();
					...
					var count  = parent.children( '.redux-repeater-accordion-repeater' ).length;
					...
					var id = parent.find( '.redux-repeater-accordion-repeater' ).length; // Index number
					...
					newSlide.find( '.redux-repeater-accordion-repeater:last .bind_title' ).on( 'change keyup', function( event ) {
						if ( $( event.target ).find( ':selected' ).text().length > 0 ) {
							var value = $( event.target ).find( ':selected' ).text();
						} else {
							var value = $( event.target ).val()
